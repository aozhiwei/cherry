...
// listHead 泛型双向循环链表头结点，同时用作链表节点。
// 空链表时next和prev指向自身；T为节点存储的数据类型。
...
// addTail 在尾部添加新节点（插入到prev和当前节点之间）
...
// firstEntry 获取链表第一个元素的数据（非头结点），空链表返回零值
...
// replaceInit 用新节点替换当前节点，并将当前节点初始化为自环
...
// empty 判断链表是否为空（只有头结点自环）
func (p *listHead[T]) empty() bool {
	return p.next == p
}

// delInit 从链表中删除当前节点并初始化为自环
...
// forEach 遍历链表，对每个非头结点的数据执行回调，返回false则提前终止。
