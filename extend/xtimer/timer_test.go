...
func newTestTimer() (*timer, *int64) {
	var now int64
	t := newTimerNoTick()
	t.Init(func() int64 { return now }, NewCascadeWheel())
	return t, &now
}
...
func TestGetMinExpires(t *testing.T) {
	...
	*now = 10
	...
	nextExpire := timer.NextExpire()
	if nextExpire != 50 {
		t.Fatalf("nextExpire should be 50, got %d", nextExpire)
	}
}
...
func newTimerNoTick() *timer {
	t := &timer{}
	w := &casadeWheel{host: t}
	t.backend = w
	return t
}
...
func BenchmarkStressTimer(b *testing.B) {
	var now int64
	tm := newTimerNoTick()
	tm.Init(func() int64 { return now }, NewCascadeWheel())

	var count int64
	tm.Every(1, func() { count++ })
...
func TestHundredThousandTimersCascade(t *testing.T) {
	...
	for r := 0; r < runs; r++ {
		...
		var now2 int64
		tm2 := newTimerNoTick()
		tm2.Init(func() int64 { return now2 }, NewCascadeWheel())
		for i := 0; i < N; i++ {
			tm2.After(256, func() { fired++ })
		}
		now2 = 255
		tm2.Update()
		now2 = 256
