...
xtimer 是 Linux 内核**多级时间轮（Hierarchical Timer5L Wheel）**的 Go 语言移植，提供 O(1) 插入/删除的高性能定时器实现。
...
使用 `Update` 驱动定时器、`NextExpire` 计算睡眠时间，实现缺省休眠、到期唤醒的事件循环：
...
for {
	t.Update()                // 1. 处理到期定时器
	idle := t.NextExpire()          // 2. 计算离下一个定时器还有多少滴答
...
**要点：**
- `Update()` 先处理到期定时器，再通过 `NextExpire()` 获取最近到期时间
- `NextExpire()` 返回绝对滴答数，调用方用 `expires - tick()` 计算睡眠时间
...
**要点：**
- 所有 `Timer5L` API（`After`/`Every`/`Delete`/`Update`/`NextExpire`）仅在专用 goroutine 内调用
- 外部 goroutine 通过 `chan cmd` 传递操作指令，channel 作为天然同步屏障
- 回调函数在专用 goroutine 内执行，如需通知外部，通过额外的 channel 发送
- `NextExpire()` 返回值可限制上限，避免无定时器时长时间阻塞 channel 接收
...
Without Handle (strong ref):
  持有者 ──→ Node ←── Timer5L
                ↑
           Timer5L 无法回收（循环引用 / 引用计数）

With Handle (weak ref):
  持有者 ──→ Handle → node(Node) ←── Timer5L
                ↕
          Delete 时 node = nil（自动断开）
```

- **避免循环引用：** 如果外部持有 `*Node` 强指针，Timer5L 回收时必须遍历所有持有者通知失效。弱引用由 Timer5L 单方面置 nil
- **自动失效：** `Delete()` 或 `Clear()` 后 `Handle.Valid()` 立即返回 false，无需外部协同
- **零泄漏：** `Handle` 仅 8 字节（一个指针），不计入 Timer5L 生命周期管理
...
| `EveryGroup` | O(1) | 同上 |
| `NextExpire` | O(1) | 返回 1 或 tick-base 差距 |
...
| 单个定时器节点 | O(1) | Node + Handle 约 200B |
...
- `Node.isDeleted()` 通过 `handle == nil` 判断定时器是否已被删除，`processTimerList` 中回调后检查此标记决定是否执行 remove/reschedule
...
| **位运算抖动** | `tick & 1023` 替代取模/随机数，单指令完成，分散不同 Timer5L 实例的 gc 触发时刻 |
...
`Timer5L` 提供 `SetOnPanic()` 注册异常处理回调，当定时器回调发生 panic 时，由用户决定吞掉还是重新抛出：
...
```
xtimer/
├── timer.go        ← Timer5L 时间轮主体（Init/Update/After/Every/NextExpire）
├── handle.go       ← Handle 句柄（Valid/Delete/Reschedule/Remain）
├── group.go        ← Group 定时器组（Clear）
├── list.go         ← 泛型侵入式双向循环链表
├── timerlist.go    ← Node 定时器节点
